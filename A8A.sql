...
PRICE_CODE INT REFERENCES PRICE(PRICE_CODE)
);
...
(
RENT_NUM  INT  NOT NULL REFERENCES RENTAL(RENT_NUM),
VID_NUM  INT NOT NULL  REFERENCES VIDEO(VID_NUM),
...
GO
--59
ALTER TRIGGER trg_mem_balance --Create Trigger name 
ON DETAILRENTAL -- on table name 
AFTER UPDATE  --when the trigger is used
AS 	
			--Declaring the variables
	DECLARE @PRIOR_LATEFEE DECIMAL;
	DECLARE @NEW_LATEFEE DECIMAL;
	DECLARE @UPDATE_AMOUNT DECIMAL;
	DECLARE	@RENTAL_MEMBER INT;
	DECLARE @VID_NUM INT;
	DECLARE @MEM_NUM INT;
if update(detail_duedate) or update(detail_returndate)
BEGIN 

SET @PRIOR_LATEFEE = (select datediff(day,detail_duedate, detail_returndate) * Detail_dailylatefee FROM deleted);
IF( @PRIOR_LATEFEE IS NULL)
BEGIN
	Set @PRIOR_LATEFEE = 0;
END 

Set @NEW_LATEFEE = (Select datediff(day,detail_duedate, detail_returndate) * Detail_dailylatefee from inserted)
IF @NEW_LATEFEE IS NULL BEGIN
SELECT @NEW_LATEFEE = 0;
END

SELECT @UPDATE_AMOUNT = @NEW_LATEFEE - @PRIOR_LATEFEE 


IF @UPDATE_AMOUNT != 0 BEGIN
Set @MEM_NUM = (SELECT M.MEM_NUM
FROM RENTAL AS R INNER JOIN MEMBERSHIP AS M ON M.MEM_NUM = R.MEM_NUM
